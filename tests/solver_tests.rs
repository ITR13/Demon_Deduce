...
fn test_puppet() {
    ...
    assert!(
        solutions.len() == 1,
        "More than 1 solution found. Solutions: {:#?}",
        solutions
    );
}

#[test]
fn test_puppet_in_deck_but_not_in_play() {
    let deck = vec![
        Role::Puppet,
        Role::Puppeteer,
        Role::Lover,
...
#[test]
fn test_puppeteer_without_puppet_not_adjacent_to_villagers() {
    let deck = vec![
        Role::Minion,
        Role::Puppeteer,
        Role::Lover,
